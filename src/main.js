...
const store = new Store({
    ...
    catalogue: {
        items: [
            ...
            { name: 'Mehl', category: 'shopping', price: 0.6 },
...
store.compute('categories', ['items'], items => Array.from(new Set(items.map(({ category }) => category))).sort());

store.compute('sumByCategory', ['items'], items => {
    const sums = {};
    items.forEach(item => {
...
store.compute('progress', ['items'], items => {
    const progresses = {};
    items.forEach(item => {
